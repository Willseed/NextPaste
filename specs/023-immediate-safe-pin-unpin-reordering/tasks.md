...
- [ ] T062 Unit test: boundary states — (a) visible list is empty after the only clip is Deleted; (b) visible list contains only the acted-on clip after a state-changing Pin or Unpin; (c) the acted-on clip is the only item in its section after a state-changing Pin (only pinned clip) or Unpin (only unpinned clip) — assert correct `sectionSortDate`, correct section placement, no crash, no duplicate UUID, and no stale snapshot retained in `NextPasteTests/ClipItemTests.swift` (FR-001, FR-002, FR-005, FR-014; Plan § Pin timestamp change)
