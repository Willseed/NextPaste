...
- [ ] T003 Add failing Swift Testing coverage for `ClipItem.isPinned == false` defaults and persisted reload behavior in `NextPasteTests/ClipItemTests.swift`
...
- [ ] T008 [P] [US1] Add UI test for row tap copy, exact clipboard text, unchanged row text, and `clip-copy-feedback` in `NextPasteUITests/ClipRowActionsUITests.swift`

### Implementation for User Story 1

- [ ] T009 [P] [US1] Add platform-specific system clipboard writer using Apple pasteboard APIs in `NextPaste/ClipboardWriter.swift`
...
- [ ] T013 [US1] Run the scoped `ClipRowActionsUITests` copy test command documented in `specs/002-clip-row-actions/quickstart.md`
...
- [ ] T015 [P] [US2] Add UI test for left swipe, `delete-clip-button`, and selected row removal in `NextPasteUITests/ClipRowActionsUITests.swift`

### Implementation for User Story 2

- [ ] T016 [US2] Add local SwiftData `deleteClip(_:)` mutation with save/rollback behavior in `NextPaste/HomeView.swift`
- [ ] T017 [US2] Add trailing swipe trash action with accessibility identifier `delete-clip-button` in `NextPaste/HomeView.swift`
...
- [ ] T020 [P] [US3] Add UI tests for right swipe, `pin-clip-button`, `pinned-clip-icon`, unpin behavior, and pinned-above-unpinned ordering in `NextPasteUITests/ClipRowActionsUITests.swift`
...
- [ ] T022 [US3] Add leading swipe pin action with accessibility identifier `pin-clip-button` in `NextPaste/HomeView.swift`
...
- [ ] T025 [P] Verify `clip-history-list`, `clip-row-{id}`, `clip-copy-feedback`, `delete-clip-button`, `pin-clip-button`, and `pinned-clip-icon` are asserted in `NextPasteUITests/ClipRowActionsUITests.swift`
...
- [ ] T027 Run the unit target command documented in `specs/002-clip-row-actions/quickstart.md`
- [ ] T028 Run the UI target command documented in `specs/002-clip-row-actions/quickstart.md`
- [ ] T029 Run the full `xcodebuild ... test` command documented in `specs/002-clip-row-actions/quickstart.md`
- [ ] T030 Record any final validation caveats or command updates in `specs/002-clip-row-actions/quickstart.md`
...
- **US1**: FR-001, FR-002, FR-003, FR-004, FR-014, FR-015, FR-019, FR-020, FR-021, FR-022.
- **US2**: FR-005, FR-006, FR-007, FR-014, FR-019, FR-020, FR-021.
- **US3**: FR-008, FR-009, FR-010, FR-011, FR-012, FR-014, FR-016, FR-017, FR-018, FR-019, FR-020, FR-021, FR-022.
...
```text
Task: T008 Add UI copy/feedback test in NextPasteUITests/ClipRowActionsUITests.swift
...
Task: T015 Add left-swipe delete UI test in NextPasteUITests/ClipRowActionsUITests.swift
```
...
Task: T020 Add pin action UI tests in NextPasteUITests/ClipRowActionsUITests.swift
```
...
2. After foundation, one developer can implement US1 while another writes US2 tests and another writes US3 tests.
