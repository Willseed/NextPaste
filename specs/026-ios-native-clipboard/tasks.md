...
relevant failure，再完成 implementation。system Paste control與App-specific設定呈現由manual matrix補足。
...
- [x] T002 在 `NextPasteUITests/UITestPathConfiguration.swift` 與 `NextPasteUITests/UITestAppLauncher.swift` 建立平台分流：iOS只傳 sandbox-relative fixture namespace／launch flags，macOS保留既有絕對隔離路徑與 real background helper
- [x] T003 在 `NextPaste/DebugUITestLaunchEnvironment.swift` 建立完整 `-ui-testing` gate下的 app-container iOS test path resolution，並在 `NextPasteTests/DebugUITestSurfaceIsolationTests.swift` 驗證 Release／不完整環境不可啟用
- [x] T004 執行 `Scripts/check-test-hygiene.sh` 並把新增／變更的 UI-test loop 審查結果同步至 `Scripts/ui-test-loop-inventory.txt`
...
- [ ] T005 [P] 在 `NextPasteTests/IOSClipboardPrivacyContractTests.swift` 先新增failing source/privacy contracts，禁止lifecycle/general-pasteboard read、clipboard write-back verification、captured content進入diagnostics/defaults/debug probe，以及任何scene/change-count checkpoint；content-free result runtime由T011驗證
- [ ] T006 [P] 在 `NextPaste/IOSClipboardImportTypes.swift` 新增content-free explicit-paste source/result/state與memory-only request ownership types，僅編譯於`os(iOS)`
...
- [x] T008 在 `NextPaste/NextPasteApp.swift` 建立精準 `#if os(iOS)` root presentation seam，移除iOS universal min-width並保留macOS/visionOS原本frame、navigation title與lifecycle wiring
- [x] T009 在 `NextPaste/Localizable.xcstrings` 建立所有新iOS navigation、paste、empty state、filter、result、editor、settings、privacy與confirmation文案的en/zh-Hant條目，不改寫既有Mac語意

**Checkpoint**: 平台presentation與content-free import types可獨立編譯，其他平台行為未改。

---

## Phase 3: User Story 1 — 複製後開啟並明確貼上目前內容（Priority: P1）🎯 MVP

**Goal**: 使用者在其他App複製文字或圖片後啟動／返回NextPaste，未點Paste前不讀取或保存；
點擊可見system PasteButton後保存最新內容一次，且不宣稱背景捕捉。

**Independent Test**: 在fake decoder與真實simulator／device各完成一次copy-before-launch flow；
launch後未點Paste為0次讀取／0筆新增，點擊後1秒內新增一筆，重複明確貼上由既有去重攔截。
...
- [ ] T011 [P] [US1] 在 `NextPasteTests/IOSClipboardImportCoordinatorTests.swift` 新增explicit provider capture、duplicate outcome、newer request replacement、cancel/stale callback、inactive/background transition survival、rapid 50 requests與content-free feedback failing tests
...
- [ ] T013 [P] [US1] 在 `NextPasteUITests/IOSExplicitPasteUITests.swift` 新增Debug fixture cold launch-before-tap zero-import、empty/nonempty Paste visibility、duplicate explicit paste與provider capture failing UI tests

### Implementation for User Story 1

- [x] T014 [US1] 在 `NextPaste/IOSPasteboardClient.swift` 實作只接受PasteButton callback `[NSItemProvider]`的async loader，依image-first規則產生既有`ClipboardPayload`並完整支援cancellation；不得存取`UIPasteboard.general`
- [ ] T015 [US1] 在 `NextPaste/IOSClipboardImportCoordinator.swift` 實作App-owned MainActor explicit request owner、in-flight replacement/cancellation、capture outcome content-free mapping與1秒量測seam
- [x] T016 [US1] 在 `NextPaste/NextPasteApp.swift` 只注入iOS coordinator並以source/runtime contract證明`.task`／`scenePhase`／notification不觸發clipboard value read；macOS `ClipboardMonitorLifecycleController`保持不變
- [x] T017 [US1] 在 `NextPaste/IOSPasteButton.swift` 封裝SwiftUI system `PasteButton(supportedContentTypes:payloadAction:)`，將explicit providers送入同一coordinator且提供本地化accessibility語意
- [x] T018 [US1] 在 `NextPaste/HomeView.swift` 提供無內容preview的iOS PasteButton primary入口與content-free feedback；空歷史由empty state顯示唯一Paste primary，已有歷史由trailing toolbar顯示
- [ ] T019 [US1] 在 `NextPaste/DebugUITestLaunchEnvironment.swift` 與 `NextPaste/Debug/UITestIOSClipboardFixture.swift` 新增完整Debug gate的deterministic explicit-provider fixture，不把實際fixture內容放入accessibility probe或Release
- [ ] T020 [US1] 更新 `NextPasteUITests/UITestAppLauncher.swift` 與 `NextPasteUITests/ClipboardFixture.swift` 讓iOS fixture只能由測試點擊Paste觸發，而macOS繼續使用real pasteboard與既有probe
...
- [x] T024 [US2] 在 `NextPaste/HomeView.swift` 以精準`#if os(iOS)`分出native home container：large navigation title、single `.searchable`、已有歷史時trailing Paste primary＋`+`secondary、低頻filter/settings menu，其他平台保留現有`AppToolbar`
- [x] T025 [US2] 在 `NextPaste/HomeView.swift` 實作iOS stable-ID `List`、active filter status/reset與`ContentUnavailableView` search/filter/history三種empty-state recovery，保留`AppTheme` canvas/card/badge/裝飾illustration
- [x] T026 [US2] 在 `NextPaste/DesignSystem/Components/ClipboardRow.swift`、`ImageClipboardRow.swift` 與 `SharedRowPresentation.swift` 增加預設不改Mac的iOS presentation參數，移除iOS窄版inline duplicate controls並支援Dynamic Type
...
- [x] T032 [US3] 在 `NextPaste/NewClipView.swift` 保留macOS body並新增iOS `NavigationStack`/scrollable editor、cancellation/confirmation toolbar、focus、interactive-dismiss draft confirmation與accessible error recovery
- [x] T033 [US3] 在 `NextPaste/HomeView.swift` 將iOS New Clip presentation接至native sheet並確保save走既有`ClipboardCaptureService`與`HistoryRetentionService`
...
- [x] T038 [US4] 在 `NextPaste/IOSSettingsView.swift` 建立native `Form` sections，直接使用`AppLanguagePreference`、`AppearancePreference`、`HistoryLimitPreference`與既有retention/history services
- [x] T039 [US4] 在 `NextPaste/IOSSettingsView.swift` 實作device-local/explicit-Paste-only／no-background-monitoring privacy說明、dynamic clear counts、destructive confirmations與About version/build，排除Mac shortcuts
- [x] T040 [US4] 在 `NextPaste/HomeView.swift` 將settings toolbar action接至native navigation destination，保留macOS `SettingsLink`與placeholder移除只限iOS
...
- [x] T042 [P] 執行 `NextPasteTests/LocalizationCatalogTests` 與catalog completeness檢查，修正 `NextPaste/Localizable.xcstrings` 所有缺漏或stale iOS/macOS translations
- [ ] T043 [P] 在 `NextPasteTests/IOSClipboardImportIntegrationTests.swift`、`NextPasteUITests/IOSExplicitPasteUITests.swift` 完成文字20次row-visible與圖片10次thumbnail／relaunch證據、callback後≤1秒量測，並在`IOSClipboardImportCoordinatorTests`完成50次explicit-request（含lifecycle transition）壓力；禁止把使用者操作時間納入app budget
...
- [x] T045 執行macOS targeted regression：`AdaptiveToolbarUITests`、`SearchAccessibilityUITests`、`SettingsUITests`、`ClipboardAutoCaptureUITests`、row-action/pin suites，確認desktop UI/polling/reconciliation parity
- [ ] T046 在booted iPhone及iPad simulator／可用實機依`quickstart.md`完成真實text/image copy→cold launch/foreground→確認未點Paste為0自動新增／0程式化prompt→點system Paste、system設定呈現；完成VoiceOver reading/actions、最大Dynamic Type、Bold Text、Increase Contrast、Reduce Transparency/Motion、Voice Control／Switch Control／hardware keyboard、Display Zoom、portrait/landscape與airplane-mode完整local flow，確認無對外clipboard content路徑，並保存不含敏感內容的screenshot/observations於repository外evidence位置
...
- [x] T048 以 `speckit-analyze` 非破壞檢查 `spec.md`、`plan.md`、`tasks.md` traceability；修正所有CRITICAL/HIGH與真實consistency defects
- [x] T049 更新 `specs/026-ios-native-clipboard/quickstart.md` 與 `contracts/validation-and-sonar-contract.md` 的最終已執行證據／accepted platform-owned limitations，保留未執行項為Pending
...
2. 完成US1，先證明copy-before-launch未點Paste不讀取，system Paste能真正保存內容一次。
...
- UI-test fixture只在完整Debug gate下；真實system paste control／setting證據必須另外執行。
...
2026-07-17：21/49 tasks具本次implementation／execution evidence；28項因缺少RED phase、
完整案例、performance/manual matrix、visionOS runtime或authoritative full-gate completion而保留Pending。
