...
- [ ] T013 [P] [US1] 在 `NextPasteUITests/IOSClipboardImportUITests.swift` 新增Debug fixture cold launch-before-tap zero-import、empty/nonempty Paste visibility、duplicate explicit paste與provider capture failing UI tests

### Implementation for User Story 1

- [ ] T014 [US1] 在 `NextPaste/IOSPasteboardClient.swift` 實作只接受PasteButton callback `[NSItemProvider]`的async loader，依image-first規則產生既有`ClipboardPayload`並完整支援cancellation；不得存取`UIPasteboard.general`
...
- [ ] T016 [US1] 在 `NextPaste/NextPasteApp.swift` 只注入iOS coordinator並以source/runtime contract證明`.task`／`scenePhase`／notification不觸發clipboard value read；macOS `ClipboardMonitorLifecycleController`保持不變
...
- [ ] T018 [US1] 在 `NextPaste/HomeView.swift` 提供無內容preview的iOS PasteButton primary入口與content-free feedback；空歷史由empty state顯示唯一Paste primary，已有歷史由trailing toolbar顯示
...
- [ ] T024 [US2] 在 `NextPaste/HomeView.swift` 以精準`#if os(iOS)`分出native home container：large navigation title、single `.searchable`、已有歷史時trailing Paste primary＋`+`secondary、低頻filter/settings menu，其他平台保留現有`AppToolbar`
- [ ] T025 [US2] 在 `NextPaste/HomeView.swift` 實作iOS stable-ID `List`、active filter status/reset與`ContentUnavailableView` search/filter/history三種empty-state recovery，保留`AppTheme` canvas/card/badge/裝飾illustration
...
- [ ] T039 [US4] 在 `NextPaste/IOSSettingsView.swift` 實作device-local/explicit-Paste-only／no-background-monitoring privacy說明、dynamic clear counts、destructive confirmations與About version/build，排除Mac shortcuts
...
- [ ] T043 [P] 在 `NextPasteTests/IOSClipboardImportIntegrationTests.swift`、`NextPasteUITests/IOSClipboardImportUITests.swift` 完成文字20次row-visible與圖片10次thumbnail／relaunch證據、callback後≤1秒量測，並在`IOSClipboardImportCoordinatorTests`完成50次explicit-request（含lifecycle transition）壓力；禁止把使用者操作時間納入app budget
