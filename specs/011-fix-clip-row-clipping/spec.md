...
- **FR-012**: The fix MUST use layout or inset correction rather than visual redesign.
...
- **FR-014**: The feature MUST include automated UI regression coverage verifying that the first visible row's full visible bounds are below the fixed header region after insertion.
- **FR-015**: The feature MUST include manual validation covering live macOS native window resizing and small, medium, and tall window heights for the top-row visibility scenario.
