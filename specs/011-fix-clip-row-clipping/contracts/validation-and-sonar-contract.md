...
- Preserve pinned-first ordering, newest-first ordering within each group, clipboard auto-capture,
  search behavior, copy/pin/delete/native swipe actions, keyboard, VoiceOver, context menus, and
...
targeted commands first and reserve full regression for final gates only.
...
2. Run targeted unit tests only for extracted pure-logic viewport/scroll-decision helpers if such a
   helper is introduced during implementation.
...
4. Keep interaction-regression UI smoke coverage for copy, pin/unpin, delete, swipe, keyboard, and
   accessibility reachability after the visibility correction.
...
| Targeted unit validation | `quickstart.md` targeted unit command(s) | If a pure-logic viewport helper is extracted, tests prove the top-inset / corrective-scroll decision rules without requiring UI launch; otherwise record “no extracted pure-logic helper” |
...
| Accessibility and platform behavior | `quickstart.md` targeted UI command(s) and interaction smoke coverage | Search field access, keyboard reachability, VoiceOver-friendly row identifiers/labels, swipe actions, and macOS-native resize behavior remain intact |
| Performance behavior | `quickstart.md` targeted UI command(s) plus manual observation | Insertion settles without perceptible repeated scroll oscillation, no persistent top gap appears, and list interaction latency remains acceptable |
...
- **Shared behavior covered by the broader run**: app launch, list rendering, searchable toolbar
  integration, clipboard capture refresh, persisted history ordering, row actions, and overall UI
  stability.
...
| Row interactions | Copy, pin/unpin, delete, native swipe actions, keyboard, context-menu, and VoiceOver behaviors remain unchanged |
...
| --- | --- | --- |
| Core user workflow | Manual clip creation at small, medium, and tall window heights | Observe the saved clip appear fully below the fixed header region immediately after insertion |
| Core user workflow | Automatic clipboard capture at small, medium, and tall window heights | Observe the captured clip appear fully below the fixed header region immediately after insertion |
...
| Pinned ordering | Pinned-first history with an insertion near the top | The first visible pinned or unpinned row still clears the fixed header region and ordering is preserved |
| Accessibility/platform behavior | Live macOS window resizing during/after insertions | The first visible row remains fully visible during size changes and interactions remain native |
...
- Affected interaction methods: native toolbar search field, mouse, trackpad, Magic Mouse,
  keyboard navigation, scrolling, context menus, native swipe actions, accessibility actions,
  VoiceOver, and live window resizing.
- Automated coverage should prove frame-based visibility, search access, and row-action reachability
  where stable identifiers already exist.
...
- Validation should confirm the list settles once without repeated jump/oscillation and without a
  permanent blank area above the first visible row.
