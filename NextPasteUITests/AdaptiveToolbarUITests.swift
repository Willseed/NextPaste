...
final class AdaptiveToolbarUITests: UITestCase {
    ...
    func testSecondaryActionsMoveToMoreMenuAndRemainKeyboardReachableOnResize() throws {
        ...
        let historyFilterMenu = app.descendants(matching: .any)["history-filter-menu"]
        XCTAssertTrue(
            historyFilterMenu.waitForExistence(timeout: ClipboardFixture.defaultTimeout),
            "Expected filter entry inside More"
        )
        historyFilterMenu.tap()

        let pinnedMenuOption = app.descendants(matching: .any)["history-filter-pinned"]
        XCTAssertTrue(
            pinnedMenuOption.waitForExistence(timeout: ClipboardFixture.defaultTimeout),
        ...
        let moreMenuAgain = app.descendants(matching: .any)["toolbar-more-menu"]
        XCTAssertTrue(
            moreMenuAgain.waitForExistence(timeout: ClipboardFixture.defaultTimeout),
        ...
        try history.createTextClip(Fixture.populatedRowsClip)
        let newClipButton = app.buttons["new-clip-button"]
        XCTAssertTrue(
            newClipButton.waitForExistence(timeout: ClipboardFixture.defaultTimeout),
        ...
        app.typeText("adaptive")
        history.assertSearchFieldContains("adaptive")
        history.clearSearch()
    }
