...
while IFS= read -r d; do
  ...
  case "$loc" in
    active) [[ "$s" =~ ^(active|draft|blocked)$ ]] || err "SPEC $id: root (active) location but status '$s'";;
    archive) [[ "$s" =~ ^(completed|deprecated|superseded|cancelled)$ ]] || err "SPEC $id: archive/ dir but status '$s'";;
    deprecated) [[ "$s" =~ ^(deprecated|superseded|cancelled)$ ]] || err "SPEC $id: deprecated/ dir but status '$s'";;
    *)
      err "SPEC $id: unrecognized location classification '$loc'"
      exit 1
      ;;
  esac
