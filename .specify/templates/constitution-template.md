<!--
Sync Impact Report
Version change: [PREVIOUS_CONSTITUTION_VERSION] -> [CONSTITUTION_VERSION]
Modified principles:
- [List updated principle titles or write "None"]
Added sections:
- [List each newly introduced governance section or write "None"]
Removed sections:
- [List removed sections or write "None"]
...
- Existing feature validation: [REPRESENTATIVE_EXISTING_FEATURE_PATH]
- Forward-generation validation: [NEW_FEATURE_PATH_OR_JUSTIFICATION]
Follow-up TODOs:
...
changes, [PROJECT_NAME] MUST automatically process the event through
`[PRIMARY_PRODUCT_TRIGGER_FLOW]` without requiring a manual save action. Manual clip creation MAY
...
service availability. New clips MUST be stored locally first using [LOCAL_STORAGE_TECHNOLOGY]
...
[APPROVED_PLATFORM_FRAMEWORKS]. Proposed dependencies or abstractions MUST be justified by a
...
[APPROVED_PLATFORM_FRAMEWORKS]. Local persistence MUST use [LOCAL_STORAGE_TECHNOLOGY], and optional
...
6. Confirm the post-implementation quality gate for the affected scope and record evidence in
   [QUALITY_GATE_EVIDENCE_LOCATION].
